...
if "%CPU%"=="ON" (
    ...
    %CMAKE% .. %CMAKE_GENERATOR% -DCMAKE_BUILD_TYPE:STRING="%BUILD_TYPE%" -DBUILD_CPU:STRING=ON %FFT_OPTIONS% %BLAS_OPTIONS% %MKL_OPTIONS%
)
if "%CUDA%"=="ON" (
...
if "%OPENCL%"=="ON" (
    echo "Running OpenCL CMake Configuration"
     %CMAKE% .. %CMAKE_GENERATOR% -DCMAKE_BUILD_TYPE:STRING="%BUILD_TYPE%" -DBUILD_OPENCL:STRING=ON -DBOOST_INCLUDEDIR:STRING="%DEPS_DIR%/boost_1_56_0" %MKL_OPTIONS%
)
@echo off

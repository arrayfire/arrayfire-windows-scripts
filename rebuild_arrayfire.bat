...
%CMAKE% .. %CMAKE_GENERATOR% -DCMAKE_BUILD_TYPE:STRING="Release" -DBUILD_CPU:STRING=OFF -DBUILD_CUDA:STRING=OFF -DBUILD_OPENCL:STRING=OFF -DBUILD_TEST:STRING=%TESTS% -DBUILD_GTEST:STRING=%TESTS% -DBUILD_EXAMPLES:STRING=%EXAMPLES% -DFREEIMAGE_FOUND:STRING=ON -DFREEIMAGE_INCLUDE_PATH:STRING="%DEPS_DIR%/freeimage-3.16.0_x64" -DFREEIMAGE_LIBRARY:STRING="%DEPS_DIR%/freeimage-3.16.0_x64/FreeImage.lib"
